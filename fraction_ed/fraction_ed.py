...
# Further modified by Anthony Lupinetti <anthony at lupinetti.us>.
...
class Fraction(numbers.Rational):
    ...
    produce a rational number equivalent to 8/6. It will NOT produce a
    reduced fraction equivalent to 4/3 like the original version of the
    library.  Both arguments must be Rational. The numerator defaults to
    0 and the denominator defaults to 1 so that Fraction(3) == 3 and
    Fraction() == 0.
    ...
    @property
    def numerator(a):
        # Property to return the top number of the fraction.
        return a._numerator

    @property
    def denominator(a):
        # Property to return the bottom number of the fraction.
        return a._denominator

    @property
    def reduced(a):
        # Property to return the a new Fraction instance the is reduced from of the fraction.
        # The rational behind doing things this way instead of using the expanded from of the
        # class call in fractions (i.e., Fractions(14,21,_True)) is that when studying fractions
        # in school, it is often desireable to show the fraction in an unreduced form and then
        # deliberately perform the reduction step.
        #
        # Examples:
        #           Fraction(10,2).reduced returns Fraction(5,1)
        #           Fraction(14,21).reduced returns Fraction(2,3)
        return Fraction(a._numerator, a._denominator, True)

    @property
    def is_improper(a):
        # Property to return a Boolean indicating whether the fraction is an impropewr fraction,
        # i.e., the numberator is greater than the denominator.
    ...
    @property
    def as_mixed(a):
        # Property to return the fraction as a mixed number.
        # if a.is_improper:
...
# This is the original Mixed library
